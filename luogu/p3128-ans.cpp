...
int lca(int x, int y) { // 求LCA，最近公共祖先
  ...
  while (depth[x] > depth[y])
    x = fa[x][lg[depth[x] - depth[y]] - 1]; // 不一步步的往根跳，而是2^i步的往根跳
  if (x == y)
    return x;
  for (int k = lg[depth[x]] - 1; k >= 0; k--) { // 不一步步的往根跳，而是2^i步的往根跳
    if (fa[x][k] != fa[y][k])
      x = fa[x][k], y = fa[y][k];
  }
  return fa[x][0];
...
void get_ans(int u, int father) {
  for (int i = head[u]; i; i = edge[i].next) {
    int to = edge[i].to;// u 的终点
    if (to == father)
      continue;
    get_ans(to, u);
    power[u] += power[to];// dfs后 power[to]里携带了to及其他的子树的关键点的加值
  }
  ans = max(ans, power[u]);
}
